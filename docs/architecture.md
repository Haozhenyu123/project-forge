...
Project Forge is a dual-harness plugin for Codex and Claude Code. It helps AI agents decide what to build and why before they start coding. It provides six skills, thirteen scripts, eight harness templates, and a coordinator flow that chains them together.
...
forge-project (skill)       -- runs scripts/forge_project.py coordinator
    |
    v
Superpowers handoff         -- export_handoff.py writes docs/superpowers-handoff.md
```
...
| `scripts/` root | `cli.py` (unified CLI), `forge_project.py` (coordinator), `smoke_test.py`, `export_handoff.py`, `install_test.py`, `clean.py`, `creative_brief.py` |
...
5. `export_handoff.py` bundles everything into `docs/superpowers-handoff.md`.
...
`scripts/mcp/server.py` implements the Model Context Protocol over stdin/stdout. It exposes nine tools: `github_search`, `web_search`, `detect_stack`, `apply_template`, `forge_project`, `export_handoff`, `validate_evidence`, `list_templates`, `run_evals`. Any MCP-compatible host can connect to it.
